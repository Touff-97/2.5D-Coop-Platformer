...
export(float) var camera_zoom_min = 2.0
export(float) var camera_zoom_max = 6.0
...
func _process(delta: float) -> void:
	...
	if player_1 != null and player_2 != null:
		...
		var cam_pos = lerp(Vector3(player_1.translation.x, player_1.translation.y + (camera_zoom - 1), camera_zoom), \
						   Vector3(player_2.translation.x, player_2.translation.y + (camera_zoom - 1), camera_zoom), \
						   camera_weight)
